...
function mmat_install() {
    ...
    $table_people = $wpdb->prefix . 'mmat_user';
    ...
    $table_status = $wpdb->prefix . 'mmat_status';
    $sql[] = "CREATE TABLE $table_status (
        id int(11) NOT NULL AUTO_INCREMENT,
        name varchar(100) NOT NULL,
        PRIMARY KEY  (id)
	) $charset_collate;";

    $table_people_activity = $wpdb->prefix . 'mmat_user_activity';
    ...
    $sql[] = "INSERT INTO $table_status (id, name) VALUES (NULL, 'Activo'), (NULL, 'Inactivo'), (NULL, 'Reactivando');";
...
function add_people_form_func(){
    ...
    $status_table = $wpdb->prefix . "mmat_status";
...
function add_activity_form_func(){
    ...
    $user_table = $wpdb->prefix . "mmat_user";

    $activities = $wpdb->get_results("SELECT * FROM $activity_table");
    $users = $wpdb->get_results("SELECT * FROM $user_table ORDER BY name");
...
function show_activity_list_func(){
    ...
    $interactions = $wpdb->get_results("
      ...
      LEFT JOIN $activity_table a ON ua.activity_id = a.id
      LEFT JOIN $status_table s ON u.status = s.id
      ORDER BY u.lname, ua.date");
...
//    ];
    ...
    foreach( $interactions as $int) {
        ...
        $tr['info']['lname']    = $int->lastname;
        $tr['info']['phone']    = $int->phone;
        $tr['info']['email']    = $int->email;
        $tr['info']['id']       = $int->id;
        $tr['info']['status']   = $int->status;
...
function add_people_form_function(){
    ...
    if ( isset( $_POST['name'] ) ){
        ...
        $tablename = $wpdb->prefix .'mmat_user';
...
function update_people_form_function(){
    ...
    if ( isset( $_POST['name'] ) ){
        ...
        $result = $wpdb->update( $tablename, $data, $where, $format, $where_format );
...
add_action('admin_post_update_people','update_people_form_function');
...
function add_activity_form_function(){
    ...
    if ( isset( $_POST['people-id'] ) ){
        ...
        $tablename = $wpdb->prefix .'mmat_user_activity';
...
function delete_user_func(){
    ...
    $wpdb->delete( $user_table, ['id' => $_POST['user_id']], ['%d'] );

    wp_die();

}

add_action( 'wp_ajax_delete_user', 'delete_user_func');
